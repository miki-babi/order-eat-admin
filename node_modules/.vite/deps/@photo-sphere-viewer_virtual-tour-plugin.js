...
// node_modules/@photo-sphere-viewer/virtual-tour-plugin/index.module.js
...
var _NodeChangedEvent = class _NodeChangedEvent2 extends TypedEvent {
  /** @internal */
  constructor(node, data) {
    super(_NodeChangedEvent2.type);
    this.node = node;
    this.data = data;
  }
};
...
var _EnterArrowEvent = class _EnterArrowEvent2 extends TypedEvent {
  /** @internal */
  constructor(link, node) {
    super(_EnterArrowEvent2.type);
    this.link = link;
    this.node = node;
  }
};
...
var _LeaveArrowEvent = class _LeaveArrowEvent2 extends TypedEvent {
  /** @internal */
  constructor(link, node) {
    super(_LeaveArrowEvent2.type);
    this.link = link;
    this.node = node;
  }
};
...
var ArrowsRenderer = class extends AbstractComponent {
  constructor(parent, plugin) {
    ...
    this.viewer.addEventListener(events_exports.PositionUpdatedEvent.type, this);
  ...
  destroy() {
    ...
    this.viewer.removeEventListener(events_exports.PositionUpdatedEvent.type, this);
  ...
  handleEvent(e) {
    switch (e.type) {
      ...
      case events_exports.PositionUpdatedEvent.type:
        this.__updateCamera();
        break;
      ...
      case "mouseenter": {
        const link = this.__getTargetLink(utils_exports.getEventTarget(e));
        if (link) {
      ...
      case "mousemove": {
        const link = this.__getTargetLink(utils_exports.getEventTarget(e), true);
        if (link) {
  ...
  render() {
    if (this.is3D) {
      ...
      this.scene.children.forEach((object) => {
        ...
        if (data.conflict) {
          const distance2 = Math.abs(utils_exports.getShortestArc(position.yaw, data.yaw));
  ...
  addLinkArrow(link, position, depth = 1) {
    ...
    if (conf.className) {
      utils_exports.addClasses(element, conf.className);
    }
    if (conf.style) {
    ...
    if (this.is3D) {
      ...
      this.scene.children.forEach((object2) => {
        ...
        if (Math.abs(utils_exports.getShortestArc(data.yaw, position.yaw)) < this.arrowsPosition.linkOverlapAngle) {
          data.conflict = true;
          conflict = true;
        }
      });
  ...
  __getTargetLink(target, closest = false) {
    const target2 = closest ? utils_exports.getClosest(target, ".psv-virtual-tour-link") : target;
...
var AbstractDatasource = class {
  ...
  checkNode(node) {
    ...
    if (!node.links) {
      utils_exports.logWarn(`Node ${node.id} has no links`);
      node.links = [];
    }
  }
  /**
  ...
  checkLink(node, link) {
    ...
    if (!this.plugin.isGps && !utils_exports.isExtendedPosition(link.position)) {
  ...
  setNodes(rawNodes) {
    ...
    rawNodes.forEach((node) => {
      if (!linkedNodes[node.id]) {
        utils_exports.logWarn(`Node ${node.id} is never linked to`);
      }
    });
    this.nodes = nodes;
  }
...
function distance(p1, p2) {
  return utils_exports.greatArcDistance(p1, p2) * 6371e3;
}
    ...
    map(map, { rawConfig }) {
      if (map) {
        if (rawConfig.dataMode === "server") {
          utils_exports.logWarn("VirtualTourPlugin: The map cannot be used in server side mode");
          return null;
        }
        if (!map.imageUrl) {
          utils_exports.logWarn('VirtualTourPlugin: configuring the map requires at least "imageUrl"');
          return null;
        }
        if (!("recenter" in map)) {
          map.recenter = true;
...
var _VirtualTourPlugin = class _VirtualTourPlugin2 extends AbstractConfigurablePlugin {
  ...
  static withConfig(config) {
    return [_VirtualTourPlugin2, config];
  }
  /**
   * @internal
   */
  init() {
    ...
    utils_exports.checkStylesheet(this.viewer.container, "virtual-tour-plugin");
    this.markers = this.viewer.getPlugin("markers");
    this.compass = this.viewer.getPlugin("compass");
    if (this.markers?.config.markers) {
      utils_exports.logWarn(
    ...
    if (!this.isServerSide) {
      ...
      if (this.config.map && !this.map) {
        utils_exports.logWarn("The map is configured on the VirtualTourPlugin but the MapPlugin is not loaded.");
      }
    }
   ...
   */
  handleEvent(e) {
    if (e instanceof events_exports.ClickEvent) {
  ...
  setNodes(nodes, startNodeId) {
    ...
    if (!startNodeId) {
      ...
      startNodeId = nodes[0].id;
      utils_exports.logWarn(`startNodeId not found is provided nodes, resetted to ${startNodeId}`);
    }
  ...
  setCurrentNode(nodeId, options, fromLink) {
    ...
    return Promise.resolve(this.state.preload[nodeId]).then(() => {
      if (this.state.loadingNode !== nodeId) {
        throw utils_exports.getAbortError();
      }
      ...
      if (utils_exports.isAbortError(err)) {
        return false;
      }
  ...
  __addNodeMarkers(node) {
    if (node.markers) {
      if (this.markers) {
        ...
        utils_exports.logWarn(`Node ${node.id} markers ignored because the plugin is not loaded.`);
      }
    }
  }
  /**
...
//# sourceMappingURL=@photo-sphere-viewer_virtual-tour-plugin.js.map
